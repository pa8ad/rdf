...
if data_format == 'xml':
    st.write(f"Totaal elementen: {len(root.findall('.//*'))}")
else:
    def cnt(o):
        if isinstance(o, dict):
            return sum(1 + cnt(v) for v in o.values())
        if isinstance(o, list):
            return sum(cnt(i) for i in o)
        return 0
    st.write(f"Totaal entries: {cnt(data)}")
